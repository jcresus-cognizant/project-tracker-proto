...
async def capture_login(browser, is_mobile):
    ...
    try:
        print(f"Capturing {prefix} Login...")
        await page.goto("http://localhost:3005/login.html")
        await page.wait_for_timeout(1000)
        await page.screenshot(path=f"{base_dir}/0_Login.png", full_page=True)
...
async def main():
    async with async_playwright() as p:
        ...
        await capture_login(browser, is_mobile=False)
        await capture_login(browser, is_mobile=True)
        await browser.close()
        print("Login screenshots captured.")
